				  ...
				  struct camera_vreg_t *cam_vreg, struct regulator **reg_ptr, int config)
{
	...
	if (config) {
		...
		*reg_ptr = regulator_get(dev, cam_vreg->reg_name);
		...
		if(!strcmp("cam_vdd_mipi_1p0_comp",cam_vreg->reg_name))
			rc = regulator_set_voltage(*reg_ptr,1000000,1000000);
		else if(!strcmp("cam_vdda_1p8_comp",cam_vreg->reg_name))
			rc = regulator_set_voltage(*reg_ptr,1800000,1800000);
		else
		#endif
			rc = regulator_set_voltage(
