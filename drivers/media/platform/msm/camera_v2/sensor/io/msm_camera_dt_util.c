...
#if defined (CONFIG_SEC_K_PROJECT) || defined(CONFIG_SEC_KACTIVE_PROJECT) || defined(CONFIG_SEC_KSPORTS_PROJECT) || defined(CONFIG_SEC_S_PROJECT) || defined(CONFIG_SEC_PATEK_PROJECT)    //To check 2P2 version
	...
	for (i = 0; i < power_setting_size; i++) {
		...
		case CAM_VDIG:
			for (j = 0; j < num_vreg; j++) {
				...
				if (!strcmp(cam_vreg[j].reg_name, "max77826_ldo1")) {
					pr_err("%s:%d i %d j %d cam_vdig\n",
					       __func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					cam_vreg[j].min_voltage = cam_vreg[j].max_voltage =
...
#if defined(CONFIG_SEC_PATEK_PROJECT)
				if (!strcmp(cam_vreg[j].reg_name, "max77826_ldo3")) {
					pr_err("%s:%d i %d j %d cam_vio\n",
					       __func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					cam_vreg[j].min_voltage = cam_vreg[j].max_voltage =
...
#if defined(CONFIG_SEC_PATEK_PROJECT)
				if (!strcmp(cam_vreg[j].reg_name, "cam_vdd_mipi_1p0_comp")) {
					pr_err("%s:%d i %d j %d cam_vdd_mipi_1p0_comp\n",
					       __func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					cam_vreg[j].min_voltage = cam_vreg[j].max_voltage =
									  power_setting[i].config_val;
...
#if defined(CONFIG_SEC_PATEK_PROJECT)
				if (!strcmp(cam_vreg[j].reg_name, "cam_vdda_1p8_comp")) {
					pr_err("%s:%d i %d j %d cam_vdda_1p8_comp\n",
					       __func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					cam_vreg[j].min_voltage = cam_vreg[j].max_voltage =
									  power_setting[i].config_val;
